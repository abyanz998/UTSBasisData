...
class MainActivity : AppCompatActivity() {
    ...
    // fungsi untuk membaca data dari database dan menampilkannya dari listview
    fun searchRecord(view: View){
        // membuat instanisasi databasehandler
        val databaseHandler: DatabaseHandler= DatabaseHandler(this)

        // memamnggil fungsi viewemployee dari databsehandler untuk mengambil data
        val emp: List<EmpModelClass> = databaseHandler.searchEmployee( )
        val empArrayId = Array<String>(emp.size){"0"}
        val empArrayName = Array<String>(emp.size){"null"}
        val empArrayEmail = Array<String>(emp.size){"null"}
        val empArrayNohp = Array<String>(emp.size){"null"}
    ...
    fun saveRecord(view: View){
        val id = u_id.text.toString() // ini dia ngambil data dulu tiap editText
        val name = u_name.text.toString()   // ini dia ngambil data juga dari editText
        val email = u_email.text.toString() // ini dia juga ngambil data dari editText
        val nohp = u_nohp.text.toString()   // ini dia juga ngambil data dari editText
        val databaseHandler: DatabaseHandler = DatabaseHandler(this)
        if(id.trim()!="" && name.trim()!="" && email.trim()!="" && nohp.trim()!=""){
            val status = databaseHandler.addEmployee(EmpModelClass(Integer.parseInt(id),name, email, nohp)) // disini datanya udah di ke variabel, dari variabel di parsing ke fungsi di DatabaseHandler
            if(status > -1){
...
//    // dia untuk mencari data  kombinasi update dan saveAdd
//    fun searchRecord(view: View){
//        val dialogBuilder = AlertDialog.Builder(this)
//        val inflater = this.layoutInflater
...
//        dialogBuilder.setView(dialogView)
//
//        val searchName = dialogView.findViewById(R.id.searchName) as EditText
//
////
//        dialogBuilder.setTitle("Search Data")
//        dialogBuilder.setMessage("Pleae fill the field")
//        dialogBuilder.setPositiveButton("Update", DialogInterface.OnClickListener { _, _ ->
//
//            val searchId = searchName.text.toString()
//
//
//            val databaseHandler: DatabaseHandler= DatabaseHandler(this)
//            if(searchId.trim()!=""){
//
//                val status = databaseHandler.searchEmployee(EmpModelSearch(searchId))
//                if(status > -1){
//                    Toast.makeText(applicationContext,"Data Updated",Toast.LENGTH_LONG).show()
//                }
//            }else{
