...
import com.example.quizsqlite01.`object`.EmpModelSearch
...
//        nama database
        private val DATABASE_NAME = "MahasiswaDatabaseKuis"

//        table beserta atributnya untuk employee
        private val TABLE_CONTACTS = "MahasiswaTable"
...
//    check apa email user satu sama dengan user yang lainnya
    ...
    // fungsi untuk menampilkan data dari tabel ke UI
    fun searchEmployee():List<EmpModelClass>{
        val empList:ArrayList<EmpModelClass> = ArrayList<EmpModelClass>()
        val selectQuery = "SELECT  * FROM $TABLE_CONTACTS WHERE $KEY_NAME LIKE 'juan' "
        val db = this.readableDatabase
        var cursor: Cursor? = null
        try{
            cursor = db.rawQuery(selectQuery, null)
